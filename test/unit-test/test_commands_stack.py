...
def test_deploy_managed_policy_stack_exists(session_manager, mocker):
    ...
    sc.deploy_managed_policy_stack(deployment_manifest=DeploymentManifest(**deployment_yaml),
...
def test_destroy_module_stack(session_manager, mocker):
    ...
    sc.destroy_module_stack(deployment_name="myapp", 
                            group_name="group",
                            module_name="module",
                            account_id="123456789012",
                            region="us-east-1",
...
def test_deploy_module_stack(session_manager, mocker):
    mocker.patch("seedfarmer.commands._stack_commands.services.cfn.does_stack_exist",return_value=(True,{"ProjectPolicyARN":"arn"}))
    mocker.patch("seedfarmer.commands._stack_commands.iam.create_check_iam_role",return_value=None)
    mocker.patch("seedfarmer.commands._stack_commands.services.cfn.deploy_template",return_value=None)
    mocker.patch("seedfarmer.commands._stack_commands.commands.seedkit_deployed", return_value=(True,"stackname", {"SeedkitResourcesPolicyArn":"arn"}))
    mocker.patch("seedfarmer.commands._stack_commands.iam.attach_policy_to_role",
    ...
    sc.deploy_module_stack(module_stack_path="test/unit-test/mock_data/modules/module-test/modulestack.yaml",
...
def test_deploy_seedkit_exists(session_manager, mocker):
    mocker.patch("seedfarmer.commands._stack_commands.commands.seedkit_deployed", return_value=(True,"stackname", {"CodeArtifactRepository":"asdfsfa"}))
    mocker.patch("seedfarmer.commands._stack_commands.commands.deploy_seedkit", return_value=None)
    sc.deploy_seedkit(account_id="123456789012",
                      region="us-east-1",
                      vpc_id="vpc-adfsfas",
...
def test_deploy_seedkit_no_exists(session_manager, mocker):
    mocker.patch("seedfarmer.commands._stack_commands.commands.seedkit_deployed", return_value=(False,"stackname", {"CodeArtifactRepository":"asdfsfa"}))
    mocker.patch("seedfarmer.commands._stack_commands.commands.deploy_seedkit", return_value=None)
    sc.deploy_seedkit(account_id="123456789012",
                      region="us-east-1",
                      vpc_id="vpc-adfsfas",
...
def test_get_module_stack_info(session_manager, mocker):
    mocker.patch("seedfarmer.commands._stack_commands.get_module_stack_names",return_value=("TheStackname","TheRoleName"))
    sc.get_module_stack_info(deployment_name="myapp", 
                            group_name="group",
                            module_name="module",
                            account_id="123456789012",
                            region="us-east-1")
